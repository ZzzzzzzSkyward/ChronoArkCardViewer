...
zzz.query = function ( name, parent, func ) {
    ...
    if ( typeof func === "function" ) {
        for ( let i = 0; i < result.length; i++ ) func( result[ i ], i );
    }
    return result;
};
...
zzz.create = function ( tag, attributes, styles, parent ) {
    ...
    if ( attributes ) {
        ...
        for ( var i in attributes ) {
            if ( attributes[ i ] !== null && attributes[ i ] !== undefined && attributes.hasOwnProperty( i ) )
...
zzz.audio = {
    ...
    create: function ( setting ) {
        ...
        var attributes = {
            muted: setting.muted || null,
            autoplay: setting.autoplay || null,
            preload: setting.preload ? "preload" : "metadata",
            controls: setting.controls || null,
            volume: setting.volume || 1
        };
        var newAudio = zzz.create( "audio", attributes );
        if ( setting.src ) {
            try {
                zzz.set( newAudio, "src", setting.src );
            } catch ( e ) {}
        }
        let result = {
            element: newAudio
...
zzz.api.download = {
    ...
    analyze: function ( targetNode ) {
        ...
        if ( validType.has( tag ) ) {
            //download this node
            let src = zzz.api.download.getsrc( targetNode );
            if ( src )
                result.push( src );
        } else {
            for ( let i of validType.keys() ) {
                ...
                if ( node ) {
                    for ( let j of node ) {
                        let src = zzz.api.download.getsrc( j );
                        result.push( src );
                    }
                }
            }
    ...
    getsrc: function ( element ) {
        ...
        let srcset = element.getAttribute( "srcset" ) || element.getAttribute( "data-srcset" );
        let hassrcset = srcset && srcset.length > 0;
        if ( hassrcset ) {
            let srcs = srcset.split( "," );
            let maxres = 0;
            let maxurl;
            for ( let i = 0; i < srcs.length; i++ ) {
                let def = srcs[ i ].strip().split( " " );
                let url = def[ 0 ];
                let res = parseInt( def[ 1 ] );
                if ( isNaN( res ) ) {
                    continue;
                }
                if ( maxres < res ) {
    ...
    batchDownloadByLocalMachine: function ( lst ) {
        let name = 1;
        let delay = 1000;
        let isTieba = zzz.api.tieba.isTieba;
        for ( let i of lst ) {
            let url = i;
            if ( isTieba ) url = zzz.api.tieba.convertImg( url );
            let send_url = zzz.path.merge( "http://localhost:5000/", {
                url: url,
                name: String( name ) || "",
                isTieba: isTieba
            } );
            zzz.time.tick( function () {
                zzz.fetch.get( send_url );
            }, delay * name );
            name++;
        }
